...
> Note: IP blocking during containment is a manual runbook step in thisbphase (adding an IP to a WAF rule via console/Terraform).
> Automated response like a Lambda function triggered by a GuardDuty or WAF finding that updates an IP set automatically is scoped for Phase 3, not built yet.
...
| Baseline WAF rules and direct Block mode for custom rate-limit rules                          | Might not be sufficient for a sophisticated attacker; Block mode without a testing/tuning period risks blocking legitimate traffic. In a real environment, Count mode would run for a few days/weeks for tuning before switching to Block. |
