...
Application and database logs are retained in CloudWatch Logs for rapid analysis with CloudWatch Logs Insights. These logs have a 30-day retention period, providing fast access to recent operational and security events. WAF logs are also ingested into CloudWatch Logs (same 30-day retention) for rule-tuning and quick investigation, while high-volume security telemetry (CloudTrail, VPC Flow Logs, ALB access logs, and AWS Config snapshots) is stored in Amazon S3 and queried with Amazon Athena.
...
WAF is associated with the ALB to protect the application against common threats: SQL injection, DDoS (rate-based limiting), bad reputation IPs, and known CVE payloads. WAF logs (BLOCK/COUNT actions only) are sent to a dedicated CloudWatch Logs group, queryable via CloudWatch Logs Insights for fast rule-tuning and investigation.
...
To detect application vulnerabilities, the ECR repository is configured with enhanced scanning and scan-on-push enabled. AWS Inspector re-scans ECR and Lambda automatically when new CVEs are discovered. The findings are routed to Security Hub.
...
- CloudWatch Logs Insights enables fast investigation of recent ECS, RDS, and WAF logs for both operational and security use cases.
...
Cloudwatch logs Insight provide quick visibility for waf(blocked & count), ecs and database logs
...
| WAF                          | AWSManagedRulesCommonRuleSet, AWSManagedRulesKnownBadInputsRuleSet, AWSManagedRulesSQLiRuleSet, AWSManagedRulesAmazonIpReputationList, global-rate-limit (4000/5min), files-posts-rate-limit (800/5min), login-rate-limit (200/5min).                                                                                                                      |
| CloudWatch Logs + CW Insight | ECS, RDS and WAF BLOCK/COUNT log, 30-day retention, queried via CloudWatch Logs Insights.                                                                                                                                                                                                                                                                  |
...
| AWS Inspector                | Enabled to ECR and Lambda, findings routed to Security Hub.                                                                                                                                                                                                                                                                                                |
...
| S3 (`logs`) + KMS            | Production-tier design: lifecycle transitions to Standard-IA at 30 days, Glacier Instant Retrieval at 90 days, Object Lock COMPLIANCE mode with 365-day retention, total object expiration at 395 days (365-day compliance lock + 30-day operational buffer for audit review before deletion). Versioning enabled, SSE-KMS with CMK, key rotation enabled. |
...
| Limited visibility on CW insight                                                              | CloudWatch log insight can only query APP, DB, and WAF (blocked/count) logs                                                                                                                                                                                                                                                                                                                              |
| VPC Flow Logs, CloudTrail, ALB access logs ingested to S3 buckets only                        | No real-time monitoring via CloudWatch Logs Insights (which requires logs to be ingested into a CloudWatch Log Group)                                                                                                                                                                                                                                                                                    |
| ELB Access Logs use SSE-S3                                                                    | Currently ELB access logs dosent support SEE-KMS encryption. A dedicated SSE-S3 bucket is used to accommodate this limitation. This cause KMS-specific security controls and auditability not available                                                                                                                                                                                                  |
...
| AWS Inspector temporary enablement                                                            | Not enabled for every deploy/destroy session to avoid per-scan cost. Enabled once to generate findings for this documentation (disabled by default). To re-enable, uncomment the resource in [inspector.tf](/infrastructure/inspector.tf).                                                                                                                                                               |
| S3 Object Lock COMPLIANCE mode + 395-day retention documented as production design only       | Compliance-mode locks can't be removed or shortened by any principal, including root, until the retention date expires. This fundamentally incompatible with this project's deploy-and-destroy lab cadence. The lab (dev) environment does not enable compliance-mode locking for this reason; this lifecycle policy is documented as the intended configuration for a persistent production deployment. |
| Limited Automation                                                                            | Currently only provide block IP in WAF, other findings still need manual investigation and eradication                                                                                                                                                                                                                                                                                                   |
